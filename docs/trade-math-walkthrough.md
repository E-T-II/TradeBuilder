...
8.5 and up is a proximal entry — buy right at Demand proximal, which is the worked example above. 7 up to just under 8.5 is a confirmation entry, where you wait for price to drop into the zone and cross back out, with the order resting 10 cents outside Demand proximal — before the zone starts, so 108.10 for this long. Below 7, the strategy calls no trade regardless of what the Decision Matrix said.
