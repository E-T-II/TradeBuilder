...
/**
 * Horizontal step indicator for large screens — a checkmark for finished
 * steps, the current step outlined, and a connecting line that's solid
 * behind completed steps and dashed ahead. Completed steps are clickable.
 */
function TopStepper({
  step,
  reachable,
...
export function TradeForm({
  ...
  return (
    <Card className="overflow-hidden p-0">
      <div className="hidden border-b px-8 py-5 lg:block">
        <TopStepper step={step} reachable={reachable} onJump={onJump} />
      </div>
      ...
      <div className="flex min-h-[70svh] flex-col p-6 lg:min-h-[460px] lg:p-8">
                ...
                <div className="grid gap-5 sm:grid-cols-2">
                  <Field id="risk" label="Risk per trade (%)" hint="2% max">
                    <Input
                      ...
                      onChange={(e) =>
                        onChange({ riskTolerance: e.target.value })
                      }
                    />
                  </Field>
                  ...
                  >
                    <Input
                      ...
                      onChange={(e) =>
                        onChange({ openTradeRisk: e.target.value })
                      }
                    />
                  </Field>
                </div>
        ...
        {/* On mobile this is a full-width action bar pinned above the score
            bar; on large screens it sits in the normal card flow. */}
        <div className="fixed inset-x-0 bottom-0 z-30 flex items-center justify-between gap-3 border-t bg-background px-6 py-3 lg:static lg:mt-8 lg:border-0 lg:bg-transparent lg:px-0 lg:py-0">
          <Button
            variant="ghost"
            onClick={onBack}
            disabled={step === 0}
